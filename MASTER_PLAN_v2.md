...
**Шаги:**
1. **Добро пожаловать** — логотип, описание
2. **Системные требования** — проверка Node.js, PostgreSQL, Redis, OLS
3. **Настройка БД** — ввод PostgreSQL credentials, тест подключения
4. **Администратор** — email, пароль, имя
5. **Настройки сервера** — домен панели, порты, email для SSL
6. **OLS интеграция** — URL, логин/пароль OLS WebAdmin
7. **Опциональные сервисы** — Telegram bot, SMTP, Aeza API
8. **Установка** — прогресс с шагами + live-терминал рядом
9. **Готово** — ссылка на панель, данные для входа

**Особенности UI:**
- Левая часть: красивый прогресс с шагами (чекбоксы с анимацией)
- Правая часть: встроенное окно терминала (xterm.js) — бегут строчки реального выполнения
- Анимации через Framer Motion
- Нельзя закрыть/перезагрузить во время установки
