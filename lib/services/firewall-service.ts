...
function parseUfwStatus(output: string): FirewallStatus {
  ...
  const defaultMatch = output.match(
  ...
  for (const line of lines) {
    ...
    // Parse rule lines like: "22/tcp  ALLOW  Anywhere"
...
export async function getStatus(): Promise<FirewallStatus> {
  ...
  if (result.exitCode !== 0) {
    throw new Error(`Failed to get UFW status: ${result.stderr}`);
  }
...
export async function enableFirewall(): Promise<void> {
  ...
  if (result.exitCode !== 0) {
    throw new Error(`Failed to enable UFW: ${result.stderr}`);
  }
}
...
export async function disableFirewall(): Promise<void> {
  ...
  if (result.exitCode !== 0) {
    throw new Error(`Failed to disable UFW: ${result.stderr}`);
  }
}
...
export async function addRule(params: {
  ...
  if (result.exitCode !== 0) {
    throw new Error(`Failed to add rule: ${result.stderr}`);
  }
  ...
  await prisma.firewallRule.create({
    data: {
      ...
      direction: "in",
      description: comment || `${action} port ${port}`,
      enabled: true,
    },
  });
}
...
export async function deleteRule(ruleNumber: number): Promise<void> {
  ...
  if (result.exitCode !== 0) {
    throw new Error(`Failed to delete rule: ${result.stderr}`);
  }
}
...
export async function setDefault(
  ...
  if (result.exitCode !== 0) {
    throw new Error(`Failed to set default: ${result.stderr}`);
  }
}
...
export async function getJailStatus(jail: string): Promise<{
  ...
  if (result.exitCode !== 0) {
    return { currentlyBanned: 0, totalBanned: 0, bannedIPs: [] };
  }
