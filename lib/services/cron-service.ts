...
function parseCrontab(output: string, user: string): CronEntry[] {
  ...
  for (const line of output.split("\n")) {
    ...
    if (trimmed.startsWith("#DISABLED#")) {
      ...
      if (match) entries.push({ expression: match[1], command: match[2], user, enabled: false });
      continue;
    }
    if (trimmed.startsWith("#")) continue;
    const match = trimmed.match(/^([\S]+\s+[\S]+\s+[\S]+\s+[\S]+\s+[\S]+)\s+(.+)$/);
    if (match) entries.push({ expression: match[1], command: match[2], user, enabled: true });
  }
  return entries;
}
...
export async function addJob(params: {
  ...
  if (fields.length !== 5) throw new Error("Invalid cron expression: must have exactly 5 fields");

  const existing = await shellSudo(`crontab -u ${user} -l`);
  const currentTab = existing.exitCode === 0 ? existing.stdout : "";
  const comment = description ? `# ${description}\n` : "";
  const newTab = `${currentTab}\n${comment}${expression} ${command}`.trim();

  const result = await shellSudo(`echo '${newTab.replace(/'/g, "'\\''")}' | crontab -u ${user} -`);
  if (result.exitCode !== 0) throw new Error(`Failed to add cron job: ${result.stderr}`);
...
export async function removeJob(params: {
  ...
  const result = await shellSudo(`echo '${filtered.replace(/'/g, "'\\''")}' | crontab -u ${user} -`);
  if (result.exitCode !== 0) throw new Error(`Failed to remove cron job: ${result.stderr}`);

  await prisma.cronJob.deleteMany({ where: { schedule: expression, command } });
}
...
export async function toggleJob(params: {
  ...
  const lines = existing.stdout.split("\n").map((line) => {
    const t = line.trim();
    if (enabled && t === disabledLine) return targetLine;
    if (!enabled && t === targetLine) return `#DISABLED#${targetLine}`;
    return line;
  });

  const result = await shellSudo(`echo '${lines.join("\n").replace(/'/g, "'\\''")}' | crontab -u ${user} -`);
  if (result.exitCode !== 0) throw new Error(`Failed to toggle cron job: ${result.stderr}`);

  await prisma.cronJob.updateMany({
    where: { schedule: expression, command },
    data: { isEnabled: enabled },
  });
}

export async function getSystemUsers(): Promise<string[]> {
  const result = await shellExec("awk -F: '$3 >= 1000 || $1 == \"root\" {print $1}' /etc/passwd");
  if (result.exitCode !== 0) return ["root"];
  return result.stdout.split("\n").filter(Boolean);
}

export const cronService = { listJobs, addJob, removeJob, toggleJob, getSystemUsers };
